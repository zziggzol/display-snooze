...
> 参考訳。正となるのは [README.md](README.md)（英語）。

![DisplaySnooze のメニュー。内蔵ディスプレイと外部の CG2420 が並んでいる](docs/screenshot.png)
...
必要なのは初回だけ。macOS 15 以降は昔の「右クリック → 開く」が効かないので、システム設定が唯一の道。それ以外は署名なしでも普通に動く。**ログイン時の自動起動も含めて**。
